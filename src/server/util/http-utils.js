...
export const buildObeservationURL = (patient, lonicCodes, url,dates) => {
    ...
    if(dates!= null && dates instanceof Array && dates.length==2){
        ...
        urlConstructed= `${url}/${Constants.OBSERVATIONS}?patient=${20003544}&code=${codes}&${dateRange}`;
    } else 
    urlConstructed= `${url}/${Constants.OBSERVATIONS}?patient=${20003544}&code=${codes}`;
    return urlConstructed;
};

export const buildMedicationURL = (patient, url) => `${url}/${Constants.MEDICATION_ORDER}?patient=${20003544}`;